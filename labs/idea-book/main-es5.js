...
function parseOutLinks(content) {
    ...
    if (matches && matches.length) {
        return matches.map(function (xs) { return xs.replace('[[', '').replace(']]', ''); });
    }
    return [];
}
var AppComponent = /** @class */ (function () {
    function AppComponent(HotkeysService) {
        var _this = this;
        this.HotkeysService = HotkeysService;
        this.showingContext = true;
        this.graph = _sample__WEBPACK_IMPORTED_MODULE_3__["SAMPLE"];
        this.HotkeysService.add(new angular2_hotkeys__WEBPACK_IMPORTED_MODULE_2__["Hotkey"]('meta+`', function (e) {
            e.preventDefault();
            _this.toggleContext();
            return false;
        }));
    ...
    AppComponent.prototype.getActiveItem = function () {
        var _a = tslib__WEBPACK_IMPORTED_MODULE_0__["__read"](this.graph.filter(function (i) { return i.active; }), 1), item = _a[0];
        return item;
    };
    AppComponent.prototype.setActiveItem = function (id) {
        // if id doesn't exist, create it
        if (!this.graph.filter(function (i) { return i.id === id; }).length) {
            this.graph.push({
                id: id,
                content: '',
                active: false
        ...
        this.graph = this.graph.map(function (i) { return (tslib__WEBPACK_IMPORTED_MODULE_0__["__assign"]({}, i, { active: i.id === id })); });
    };
    AppComponent.prototype.getItemsLinkedTowardThis = function () {
        var id = this.getActiveItem().id;
        return this.graph.filter(function (i) { return i.id !== id && parseOutLinks(i.content).indexOf(id) > -1; }).map(function (i) { return i.id; });
    };
    AppComponent.prototype.getItemsLinkedAwayFromThis = function () {
        return parseOutLinks(this.getActiveItem().content || '').sort();
    };
    AppComponent.prototype.outputGraph = function () {
